...
let ds = [];
let cutoff = 0; // max number of droplets to simulate
// Numerical parameters
const dt = 0.1; // s, time step | I adjusted this for the 100ms loop time
...
const h_0 = (_Window.height - _Helper.m_to_px(10)) - 10; // firing height (1.5 m) | I updated this to get the meter distance in pixels for the bottom of the window - KS

const theta_0 = (35.0 * Math.PI) / 360.0; // radians, firing angle above horizontal
...
const update = () => {
    ...
    for (let d of ds) {
        d.t = d.t + dt; // update time since start of simulation
        ...
        d.velocity.v = d.velocity.v + dt * (g + drag_y);
    ...
    // Cut off after 50 frames. To stop my browser from crashing.
    cutoff++;
    // if (cutoff > 100) {
    //     kill_sim();
    // }
